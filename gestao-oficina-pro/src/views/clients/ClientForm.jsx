...
const ClientForm = ({ formData, onChange, onSubmit, isSubmitting, title }) => {
    return (
        <form onSubmit={onSubmit}>
            ...
            <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-color">
                <button type="submit" className="btn btn-primary px-8" disabled={isSubmitting}>
                    {isSubmitting ? <Loader className="animate-spin" size={18} /> : (title.includes('Editar') ? 'Salvar Alterações' : 'Cadastrar Cliente')}
                </button>
            </div>
        </form>
    );
